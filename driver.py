...
class Driver(object):
    ...
    def train_forward_model(self):
        ...
        # print('train_f_m: ', states_[0], states[0], actions[0])
    ...
    def train_discriminator(self):
        ...
        # print('train_disc: ', state_a_[0], action_a[0], state_e_[0], action_e[0])
        if self.env.gan_type == 1:
            run_vals = self.sess.run(fetches, feed_dict)
        if self.env.gan_type == 2:
            clip_D = [p.assign(tf.clip_by_value(p, -0.01, 0.01)) for p in alg.discriminator.weights]
            fetches = [alg.discriminator.minimize, alg.discriminator.loss, alg.discriminator.acc, clip_D]
            run_vals = self.sess.run(fetches, feed_dict)
        if self.env.gan_type == 3:
            feed_dict = {alg.states: states, alg.actions: actions,
                     alg.label: labels, alg.do_keep_prob: self.env.do_keep_prob,
                         alg.actions_expert:action_e, alg.states_expert: state_e_,
                         alg.actions_agent:action_a, alg.states_agent: state_a_}
            run_vals = self.sess.run(fetches, feed_dict)
    ...
    def collect_experience(self, record=1, vis=0, n_steps=None, noise_flag=True, start_at_zero=True):
        ...
        while not done:
            ...
            if not noise_flag:
                do_keep_prob = 1.
            # print('collect_experience:', np.reshape(observation, [1, -1])[0])
            ...
            # print('test action: ', a[0])
            # print('test state:', np.reshape(observation, [1, -1])[0])
            ...
            if self.itr < self.env.prep_time:
                ...
                if self.discriminator_policy_switch:
                    ...
                    if self.env.gan_type == 2 or self.env.gan_type == 3:
                        for t_discriminator in range(1,4):
                            self.train_discriminator()
                else:
                    self.train_policy()
    ...
    def print_info_line(self, mode):
        if mode == 'full':
            ...
            if self.results == []:
                self.results = np.array([[self.itr, self.reward_mean, self.reward_std]])
            else:
                self.results = np.concatenate(
                    (self.results, np.array([[self.itr, self.reward_mean, self.reward_std]])))

        else:
            buf = "processing iter: %d, loss(f_m,disc,p): %s\n" % (self.itr, self.loss)
        sys.stdout.write('\r' + buf)
    ...
    def save_model(self, dir_name=None):
        ...
        results_fname_ = self.run_dir + '/results/' + self.env.results_fname + time.strftime("%Y-%m-%d") + '.npy'
        np.save(results_fname_, self.results)
