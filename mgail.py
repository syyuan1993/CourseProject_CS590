...
class MGAIL(object):
    def __init__(self, environment):
        ...
        self.states_agent = tf.placeholder("float", shape=(None, self.env.state_size), name='states_')  # Batch x State
        self.states_expert = tf.placeholder("float", shape=(None, self.env.state_size), name='states')  # Batch x State
        self.actions_agent = tf.placeholder("float", shape=(None, self.env.action_size), name='action')
        self.actions_expert = tf.placeholder("float", shape=(None, self.env.action_size), name='action')
        ...
        if self.env.GRU:
            ...
            forward_model_prediction, _ = self.forward_model.forward([states_, actions, initial_gru_state], gru_used=self.env.GRU)
        else:
            forward_model_prediction, _ = self.forward_model.forward([states_, actions], gru_used=self.env.GRU)
        ...
        # 2.2 prediction
        if self.env.gan_type == 1: # vanilla GAN
        ...
        if self.env.gan_type == 2: # WGAN
            # expert d[:,1]
            # agent d[:,0]
            # label: 0: agent, 1: expert
            d_loss_weighted = self.env.cost_sensitive_weight * tf.multiply(tf.to_float(tf.equal(tf.squeeze(self.label), 0.)), d)- \
                                  tf.multiply(tf.to_float(tf.equal(tf.squeeze(self.label), 1.)), d)



        if self.env.gan_type == 3: # WGAN-GP
            alpha = tf.random_uniform(
                shape=[self.env.batch_size, 1],
                minval=0.,
                maxval=1.
                )
            d_loss_weighted = self.env.cost_sensitive_weight * tf.multiply(tf.to_float(tf.equal(tf.squeeze(self.label), 0.)), d)- \
                              tf.multiply(tf.to_float(tf.equal(tf.squeeze(self.label), 1.)), d)
            d_loss_weighted = tf.reduce_mean(d_loss_weighted)

            differences_states = self.states_agent - self.states_expert
            differences_actions = self.actions_agent - self.actions_expert

            interpolate_states = self.states_expert + (alpha*differences_states)
            interpolate_actions = self.actions_expert + (alpha * differences_actions)

            gradients = tf.gradients(self.discriminator.forward(interpolate_states, interpolate_actions, reuse=True),[interpolate_states, interpolate_actions])[0]
            slopes = tf.sqrt(tf.reduce_sum(tf.square(gradients), reduction_indices=[1]))
            gradient_penalty = tf.reduce_mean((slopes - 1.) ** 2)
            d_loss_weighted += self.env.LAMBDA * gradient_penalty
        ...
        # 3. Collect experience from forward model
        ...
        def policy_loop(state_, t, total_cost, total_trans_err, _):
            ...
            if self.env.GRU == True:
                state_a, _ = self.forward_model.forward([state_, action, initial_gru_state], reuse=True)
            else:
                state_a, _ = self.forward_model.forward([state_, action], reuse=True, gru_used=self.env.GRU)
    ...
    def al_loss(self, d):
        if self.env.gan_type == 1:
            ...
            if self.env.gan_type == 2 or self.env.gan_type == 3:
