...
class Policy(object):
    def __init__(self, in_dim, out_dim, size, lr, do_keep_prob, n_accum_steps, weight_decay, gan_type = 1):
        ...
        self.gan_type= gan_type # 1:vanilla GAN, 2: WGAN
        self.solver_params = {
            'lr': lr,
    ...
    def backward(self, loss):
        ...
        if self.gan_type==2:
            opt = tf.train.RMSPropOptimizer(learning_rate=self.solver_params['lr'])
        if self.gan_type == 3:
            opt = tf.train.AdamOptimizer(learning_rate=self.solver_params['lr'], beta1=0.5, beta2=0.9)
        ...
        if self.gan_type == 1:
            ...
            grads = [tf.clip_by_value(g, -100, 100) for g, v in grads_and_vars]
